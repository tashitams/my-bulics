<template>
    <div>
        <meta-head title="Add Health Record" />
        <!-- breadcrumb -->
        ...
        <div class="px-2">
            <h3 class="font-bold text-lg mb-4">Health Record Form</h3>
            <form @submit.prevent="store" autocomplete="off">
                <div class="space-y-6">
                    <div class="grid grid-cols-1 gap-20 gap-y-8">
                        <!-- title -->
                        <div>
                            <label for="title" class="form-label">Sickness</label>
                            <input v-model="form.title" type="text" placeholder="eg. Headache, Bodyache" class="form-input" :class="{ 'has-error': errors.title }" id="title" required />
                            <div v-if="errors.title" class="form-error">{{ errors.title }}</div>
                        </div>
                        <!-- description -->
                        <div>
                            <label for="description" class="form-label">Description</label>
                            <textarea v-model="form.description" id="description" maxlength="200" placeholder="Write what did you do to help the sick student..." class="form-input"></textarea>
...
export default {
    ...
    methods: {
        store() {
            this.form.post(this.route('staff.students.health', this.student.id))
        },
    },
}
